...
import { Filter, FilterX, PanelLeftClose, PanelLeftOpen } from "lucide-react";
import * as React from "react";

import SlideTransition from "@/components/ui/SlideTransitions";
...
>(
  (
    ...
    return (
      <SlideTransition show={state === "expanded"} direction="left">
        <div
          ref={ref}
          className={cn("group peer hidden md:block text-sidebar-foreground border-r")}
          data-state={state}
        ...
        >
          ...
          <div
            className={cn(
              "duration-200 relative inset-y-0 z-10 hidden w-[--sidebar-width] transition-[left,right,width] ease-linear md:flex",
              side === "left"
...
const SidebarTrigger = React.forwardRef<
  ...
  React.ComponentProps<typeof Button> & { sidebarVariant?: "default" | "filter" }
>(({ className, onClick, sidebarVariant = "default", ...props }, ref) => {
      ...
      {sidebarVariant === "filter" ? (
        <>
          ...
          <span className="hidden md:inline-block">
            {state !== "expanded" ? "Show Filters" : "Hide Filters"}
          </span>
        </>
      ) : state !== "expanded" ? (
        <PanelLeftOpen size={24} opacity={0.8} />
      ) : (
        <PanelLeftClose size={24} opacity={0.8} />
      )}
